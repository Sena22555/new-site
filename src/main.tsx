...
function ParallaxImage() {
  const ref = useRef<HTMLDivElement>(null); const [offset, setOffset] = useState(0);
  useEffect(() => { const onScroll = () => { if (ref.current) { const r = ref.current.getBoundingClientRect(); setOffset(Math.max(-35, Math.min(35, (innerHeight / 2 - r.top) * .08))); } }; onScroll(); addEventListener('scroll', onScroll, { passive: true }); return () => removeEventListener('scroll', onScroll); }, []);
...
function Partner() {
  ...
  return <section className="partner wide" onMouseMove={addThumb}><div className="partner-inner"><p className="eyebrow">The right kind of weird</p><h2>Partner with<br /><em>the odd ones.</em></h2><p className="partner-copy">A small, senior team for the moments when good enough is nowhere near enough.</p><a className="text-link" href="mailto:hello@viktoroddy.com">Start a conversation <ArrowUpRight size={15} /></a>{thumbs.map((t) => <img className="cursor-thumb" key={t.id} src={t.src} alt="" style={{ left: t.x, top: t.y, transform: `translate(-50%, -50%) rotate(${t.rotate}deg)` }} />)}</div></section>;
}
function App() {
